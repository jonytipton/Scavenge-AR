//
//  CreateAnchorViewController.swift
//  Scavenge[AR]
//
...
class CreateAnchorViewController: BaseViewController, UITextFieldDelegate {

    @IBOutlet var sensorStatusView: SensorStatusView!
    
    @IBOutlet var anchorNameField: UITextField!

    
    override func viewDidLoad() {
        anchorNameField.delegate = self //set delegate
        super.viewDidLoad()
    }
    
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        anchorNameField.resignFirstResponder() //dismiss keybaord
        anchorNameField.isHidden = true //hide textfield
        
        if textField.text != nil {
            print("Inputted Anchor Name: " + textField.text!)
            super.anchorName = textField.text ?? "NIL"
        }
        else {
            print("input name is NIL! setting 'defaultName' as anchor name")
        }
        nameSet = true
    ...
    private var nameSet : Bool = false
    ...
    override func onNewAnchorLocated(_ newCloudAnchor: ASACloudSpatialAnchor) {
        ignoreMainButtonTaps = false
        step = .stopWatcher
    ...
    @objc override func mainButtonTap(sender: UIButton) {
        ...
        case .prepare:
            ...
            step = .nameAnchor
        case .nameAnchor:
            anchorNameField.isHidden = false
            self.mainButton.setTitle("Save Anchor Name", for: .normal)
    ...
    private func createLocationProvider() {
        ...
        locationProvider!.sensors!.knownBeaconProximityUuids =
            CreateAnchorViewController.knownBluetoothProximityUuids
    ...
    private func enableAllowedSensors() {
        if let sensors = locationProvider?.sensors {
            ...
            sensors.wifiEnabled = CreateAnchorViewController.haveAccessWifiInformationEntitlement
